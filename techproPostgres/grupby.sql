...
group by urun_adi
--2.yol
select urun_adi  as urunadi, count(isim) as kisisayisi from manav 				
where urun_adi is not null				
group by urun_adi				
...
select ulke,cinsiyet, count(*), avg(yas) as yas_ortlaması from personel group by ulke, cinsiyet
